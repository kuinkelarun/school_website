...
export default function SiteSettingsPage() {
  ...
  // Track whether the form has been initialised from Firestore yet.
  // We only want to populate ONCE so that user edits are never silently
  // overwritten by a subsequent settings reference change or re-render.
  const settingsInitialised = useRef(false);

  // Populate form when settings load (first time only)
  useEffect(() => {
    if (settings && !settingsInitialised.current) {
